...
template loadString(parser, data) =
   if parser.tok == tkNull:
      ...
      raiseParseErr(parser, "string or null")
...
proc loadAnyImpl(nodeTy, param, parser, genProcs: NimNode, cache: TableRef[
      string, NimNode], isRef: bool, depth: int): NimNode =
   let baseTy = getTypeImpl(nodeTy)
   case baseTy.typeKind
   of ntyRef:
      let res = ident"result"
      let parserPar = genSym(nskParam, "p")
      let name = genSym(nskProc, "packImpl")
      cache[nodeTy.repr] = name
      genProcs.add newProc(name, [nodeTy, newIdentDefs(parserPar,
            nnkVarTy.newTree(bindSym"JsonParser"))], newStmtList(newCall(
            bindSym"new", res), loadAny(baseTy[0], res, parserPar, genProcs,
            cache, true, depth + 1)), pragmas = nnkPragma.newTree(
                  ident"nimcall"))
   ...
   of ntyObject, ntyTuple:
      ...
      for n in idents:
         ...
         caseStr.add nnkOfBranch.newTree(newLit(n[0].strVal), newStmtList(
               getAst(getFieldValue(parser2)), loadAny(n[1], newDotExpr(res,
               n[0]), parser2, genProcs, cache, false, depth + 1)))
      caseStr.add nnkElse.newTree(getAst(raiseWrongKey(parser2)))
      if isRef:
         result = getAst(loadObj(parser2, caseStr))
      else:
         let name = genSym(nskProc, "packImpl")
         cache[nodeTy.repr] = name
   ...
   of ntySet, ntySequence:
      ...
      let body = loadAny(baseTy[1], temp, parser, genProcs, cache, false,
            depth + 1)
      let addTemp = newCall(if baseTy.typeKind ==
            ntySet: bindSym"incl" else: bindSym"add", param, temp)
      result = getAst(loadSeq(parser, newStmtList(initTemp, body, addTemp)))
   of ntyArray:
      let idx = genSym(nskVar, "i")
      let body = loadAny(baseTy[2], nnkBracketExpr.newTree(param, idx),
            parser, genProcs, cache, false, depth + 1)
...
macro to*(s: Stream, T: typedesc): untyped =
   ...
   let body = loadAny(typeSym, ident"result", parser, genProcs, cache, false, 0)
   result = nnkStmtListExpr.newTree(genProcs, getAst(genPackProc(parser,
         typeSym, name, body)), newCall(name, s))
